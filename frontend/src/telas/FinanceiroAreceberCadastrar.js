...
const formasRecebimento = ["Boleto", "Transferência", "Cartão", "Pix", "Dinheiro"];
...
function FinanceiroAreceberCadastrar() {
  const [cliente, setCliente] = useState("");
  const [descricao, setDescricao] = useState("");
  const [valor, setValor] = useState("");
  const [dataVencimento, setDataVencimento] = useState("");
  const [numParcelas, setNumParcelas] = useState(1);
  const [formaRecebimento, setFormaRecebimento] = useState(formasRecebimento[0]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!cliente || !descricao || !valor || !dataVencimento || !numParcelas || !formaRecebimento) {
    ...
    try {
      ...
      setCliente(""); setDescricao(""); setValor(""); setDataVencimento(""); setNumParcelas(1); setFormaRecebimento(formasRecebimento[0]); setObservacoes("");
    } catch (err) {
        ...
        <div className="col-12 col-md-10 col-lg-8">
          <div className="card bg-dark text-white shadow-lg border-0 animate__animated animate__fadeInUp">
            <div className="card-body p-5">
              <h3 className="mb-4 fw-bold text-center text-uppercase letter-spacing-1">Cadastro de Conta a Receber</h3>
              <form onSubmit={handleSubmit} autoComplete="off">
                <div className="row g-4">
                  <div className="col-md-6">
                    <label className="form-label text-white fw-semibold">Cliente *</label>
                    <select className="form-select bg-black text-white border-secondary shadow-sm" value={cliente} onChange={e => setCliente(e.target.value)} disabled={loading || parceiros.length === 0} required>
                  ...
                  <div className="col-md-2">
                    <label className="form-label text-white fw-semibold">Recebimento *</label>
                    <select className="form-select bg-black text-white border-secondary shadow-sm" value={formaRecebimento} onChange={e => setFormaRecebimento(e.target.value)} disabled={loading}>
...
export default FinanceiroAreceberCadastrar; 